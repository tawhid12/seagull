...
@section('pageTitle','Upload Invoice Report')
@section('pageSubTitle','Upload Invoice Report')
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th colspan="8" class="text-center">
                                    <h5>Upload Invoice Report</h5>
                                </th>
                            </tr>
                            <tr>
                                <th colspan="8" class="text-center">
                                    <form class="form" method="post" enctype="multipart/form-data" action="{{route('invoice-report.store')}}">
                                        @csrf
                                        <input type="hidden" name="order_id" value="{{request()->get('id')}}">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <input type="file" class="form-control" name="invoice_report">
                                                </div>
                                                @if($errors->has('invoice_report'))
                                                <span class="text-danger"> {{ $errors->first('invoice_report') }}</span>
                                                @endif
                                            </div>
