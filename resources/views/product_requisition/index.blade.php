                ...
                <div class="card">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered mb-0">
                            <a class="btn btn-sm btn-primary float-end" href="{{ route('product-requisition.create') }}"><i
                            ...
                            <tbody>
                                @forelse($product_requisitions as $pr)
                                    <tr>
                                        ...
                                        <td>{{ $pr->title }}</td>
                                        <td>{{ $pr->order?->order_subject }}</td>
                                        <td>{{ $pr->order?->company?->company_name }}</td>
                                        ...
                                        <td class="white-space-nowrap">
                                            <a
                                                href="{{ route('product-requisition.edit', encryptor('encrypt', $pr->id)) }}">
                                            ...
                                            <form id="form{{ $pr->id }}"
                                                action="{{ route('product-requisition.destroy', encryptor('encrypt', $pr->id)) }}"
                                                method="post">
                                                @csrf
                                                @method('delete')
